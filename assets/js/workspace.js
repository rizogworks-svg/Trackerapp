...
function renderDetailExtras(s){
  ...
  $('dTaskPreview').innerHTML=tasks.slice(0,6).map(t=>`<div><div class="task-preview-copy"><strong>${esc(t.title)}</strong>${t.date?`<small>Tanggal: ${esc(fmt(t.date))}</small>`:''}${t.dueDate?`<small>Tenggat: ${esc(fmt(t.dueDate))}</small>`:''}</div><span>${t.status==='Completed'?'✓ Selesai':t.status==='In Progress'?'Proses':'Pending'}</span></div>`).join('');
