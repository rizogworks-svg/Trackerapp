...
self.addEventListener('fetch',e=>{
 ...
 if(req.mode==='navigate'){
   e.respondWith(fetch(req).then(async r=>{if(r.ok){const c=await caches.open(CACHE_NAME);await c.put('./index.html',r.clone());}return r;}).catch(async()=>{const c=await caches.open(CACHE_NAME);return await c.match('./index.html')||new Response('Aplikasi belum tersedia offline.',{status:503});}));return;
 }
 if(!APP_SHELL.some(p=>new URL(p,self.registration.scope).pathname===url.pathname))return;
 e.respondWith(caches.open(CACHE_NAME).then(async c=>await c.match(req,{ignoreSearch:true})||fetch(req)));
});
