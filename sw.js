...
const CACHE_NAME=CACHE_PREFIX+'v7.5';
...
const VERSIONED_SHELL=APP_SHELL.map(p=>p==='./'||p==='./index.html'||p.includes('/icons/material/')?p:p+'?v=20260923.75');
const LIBRARIES=new Set(['https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.57.5/dist/umd/supabase.js','https://unpkg.com/@supabase/supabase-js@2.57.5/dist/umd/supabase.js','https://cdn.sheetjs.com/xlsx-0.20.3/package/dist/xlsx.full.min.js']);
...
self.addEventListener('message',e=>{if(e.data?.type==='SKIP_WAITING')self.skipWaiting();if(e.data?.type==='GET_VERSION')e.ports[0]?.postMessage('7.5');});
